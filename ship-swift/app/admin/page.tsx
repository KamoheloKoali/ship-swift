...
import { getUnverifiedDrivers, updateDriverVerification } from '@/actions/driverActions'
...
export default function AdminDashboard() {
  ...
  useEffect(()=>{
    const fetchUsers = async () => {
      const [clients, drivers] = await Promise.all([getUnverifiedClients(), getUnverifiedDrivers()])
  ...
  const handleVerify = async (userId: string, user: any) => {
    ...
    if (user.type === 'client') {
      ...
      // verify driver
      const response = await updateDriverVerification(userId)
  ...
  const pendingClients = users.filter(user => user.type === 'client' && user.status === 'pending')
  const pendingCouriers = users.filter(user => user.type === 'courier' && user.status === 'pending')
  ...
  const UserCard = ({ user }: { user: User }) => (
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <img src={user.idImage} alt="ID Document" className="w-full h-40 object-cover rounded-md" />
          </div>
          <div>
            <p className="text-sm font-medium mb-2">Selfie</p>
            <img src={user.selfieImage} alt="Selfie" className="w-full h-40 object-cover rounded-md" />
          </div>
          {user.type === 'courier' && (
            <div className="col-span-2">
              <p className="text-sm font-medium mb-2">Driver's License</p>
              <img src={user.driverLicenseImage} alt="Driver's License" className="w-full h-40 object-cover rounded-md" />
            </div>
          )}
        </div>
      </CardContent>
      <CardFooter className="flex justify-between">
        ...
        <Button variant="default" onClick={() => handleVerify(user.id, user)}>
          ...
          <ScrollArea className="h-[calc(100vh-200px)]">
            <div className="grid gap-6">
              ...
              {pendingClients.length === 0 && (
                <p className="text-center text-muted-foreground">No pending clients to verify.</p>
              )}
            </div>
          </ScrollArea>
        </TabsContent>
          ...
          <ScrollArea className="h-[calc(100vh-200px)]">
            <div className="grid gap-6">
              ...
              {pendingCouriers.length === 0 && (
                <p className="text-center text-muted-foreground">No pending couriers to verify.</p>
              )}
            </div>
          </ScrollArea>
        </TabsContent>
